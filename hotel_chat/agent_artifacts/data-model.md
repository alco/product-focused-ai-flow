...
| dm_member_a              | uuid        | yes  |                   | DM pair, canonically ordered (CHECK `dm_member_a < dm_member_b`); set iff kind='dm' (CHECK); composite FK `(id, dm_member_a)` → `conversation_members(conversation_id, member_id)` DEFERRABLE INITIALLY DEFERRED |
| dm_member_b              | uuid        | yes  |                   | second half of the DM pair; same CHECKs/FK as `dm_member_a`                                                                  |
...
**Indexes:** `(company_id)`; **partial unique** `(company_id, dm_member_a, dm_member_b) WHERE kind = 'dm'` (one DM per pair, race-proof); partial `(company_id, kind) WHERE kind IN ('location_channel','company_channel')` (channel auto-provisioning checks).
...
| 6   | `dm_member_a`/`dm_member_b` + partial unique index = race-proof one-DM-per-pair without a junction lookup; a DM pair has no single-row representation elsewhere (it spans two `conversation_members` rows), so the pair is materialized on the conversation row where a unique index can see it. Originally a text `dm_key` whose format was convention-only — replaced with real columns + CHECKs + deferred composite FKs after seeds wrote slug pairs and escaped the constraint (session 7).                                                                                                                                                                                                                                                                                                                                                                                                                                             |
